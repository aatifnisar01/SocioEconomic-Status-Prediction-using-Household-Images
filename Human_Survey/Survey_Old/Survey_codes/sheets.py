# -*- coding: utf-8 -*-
"""
Created on Sun Apr  2 13:14:57 2023
...
creds = Credentials.from_service_account_file('C:\\Users\\Aatif\\Downloads\\interior-382406-6056245748f2.json', scopes=scope)
client = gspread.authorize(creds)

# Replace the string with the ID of your Google Sheets document
spreadsheet_id = '1WS9-e7s_abBKRQiIXrH1SnGDHG14R1vImI98dEen4zY'

# Access the first sheet of the document
sheet = client.open_by_key(spreadsheet_id).sheet1

# Example of updating a cell value
sheet.update_cell(1, 1, "Image Name")
sheet.update_cell(1, 2, "Class_Name")
sheet.update_cell(1, 3, "Selected_Folder")
sheet.update_cell(1, 4, "Comment")
...
for folder, label in folders.items():
    folder_path = f'C:/AHI data/Interior/{folder}'
...
image_frame.place(relx=0.13, rely=0.25)
...
comment_frame.place(relx=0.5, rely=0.8, relwidth=0.45, relheight=0.1)
...
comment_label.pack(padx=5, pady=5, side='left')
comment_entry = Entry(comment_frame)
comment_entry.pack(padx=0, pady=5, side='right', fill='x', expand=True)
...
def next_image(selected_folder):
    ...
    selected_folder_path = f'C:/AHI data/Interior/class{selected_index}'
    ...
    if comment is None:
        # If this is the first image and no comment has been entered, do not write to the Google Sheet
        pass
    elif not comment:
        ...
        # Write the comment to the Google Sheet
        row = [image_name, class_label, selected_folder, comment]
