# -*- coding: utf-8 -*-
"""
Created on Tue Apr 25 22:34:29 2023
...
y_1 = (np.percentile(list_incomes_norm,12.5))
y_2 = (np.percentile(list_incomes_norm,25))
y_3 = (np.percentile(list_incomes_norm,37.5))
y_4 = (np.percentile(list_incomes_norm,50))
y_5 = (np.percentile(list_incomes_norm,62.5))
y_6 = (np.percentile(list_incomes_norm,75))
y_7 = (np.percentile(list_incomes_norm,87.5))
...
for i in range(len(list_incomes_norm)):
    ...
    elif list_incomes_norm[i]<=y_5:
        li_4.append([list_images_incomes[i][0],list_incomes_norm[i]])
    elif list_incomes_norm[i]<=y_6:
        li_5.append([list_images_incomes[i][0],list_incomes_norm[i]])
    elif list_incomes_norm[i]<=y_7:
        li_6.append([list_images_incomes[i][0],list_incomes_norm[i]])
        
    else:
        li_7.append([list_images_incomes[i][0],list_incomes_norm[i]])
        
li = [li_0, li_1, li_2,  li_3,  li_4, li_5, li_6, li_7]
print ('Sum of lengths: ', len(li_0)+len(li_1)+len(li_2)+len(li_3)+len(li_4)+len(li_5)+len(li_6)+len(li_7), len(list_incomes_norm))
...
for i in range(8):
    ...
    for x in tqdm(train[i]):
        filename = x[0]
        old_file = 'C:/AHI Data/Interior_Split8/' + filename + '_HouseInterior_6.jpg'
        new_fold = 'C:/AHI Data/Interior_Split8/train/class'+str(i)+'/'
        try:
    ...
    for x in tqdm(test[i]):
        filename = x[0]
        old_file = 'C:/AHI Data/Interior_Split8/' + filename + '_HouseInterior_6.jpg'
        new_fold = 'C:/AHI Data/Interior_Split8/test/class'+str(i)
        try:
            os.makedirs(new_fold)
        except:
            pass
    ...
    for x in tqdm(valid[i]):
        filename = x[0]
        old_file = 'C:/AHI Data/Interior_Split8/' + filename + '_HouseInterior_6.jpg'
        new_fold = 'C:/AHI Data/Interior_Split8/valid/class'+str(i)
        try:
            os.makedirs(new_fold)
        except:
            pass
