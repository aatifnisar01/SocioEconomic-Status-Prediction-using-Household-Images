...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug  1 12:16:30 2022
...
def swin_transformer_tiny_224_w7_1k(input_tensor = None, input_shape = None, window_size = 7, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
...
def swin_transformer_tiny_224_w7_22k(input_tensor = None, input_shape = None, window_size = 7, classes = 21841, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.1, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_tiny_224_w7_22kto1k(input_tensor = None, input_shape = None, window_size = 7, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.1, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_small_224_w7_1k(input_tensor = None, input_shape = None, window_size = 7, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.3, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_small_224_w7_22k(input_tensor = None, input_shape = None, window_size = 7, classes = 21841, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_small_224_w7_22kto1k(input_tensor = None, input_shape = None, window_size = 7, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_base_224_w7_1k(input_tensor = None, input_shape = None, window_size = 7, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.5, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = swin_transformer(img_input, classes, include_top, patch_size = 4, n_feature = 128, n_blocks = [2, 2, 18, 2], n_heads = [4, 8, 16, 32], window_size = window_size, ratio = 4., scale = None, use_bias = True, patch_normalize = True, dropout_rate = dropout_rate, attention_dropout_rate = attention_dropout_rate, droppath_rate = droppath_rate, normalize = normalize, activation = activation)
    model = tf.keras.Model(img_input, out)
...
def swin_transformer_base_224_w7_22k(input_tensor = None, input_shape = None, window_size = 7, classes = 21841, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_base_224_w7_22kto1k(input_tensor = None, input_shape = None, window_size = 7, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_base_384_w12_1k(input_tensor = None, input_shape = None, window_size = 12, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.5, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_base_384_w12_22k(input_tensor = None, input_shape = None, window_size = 12, classes = 21841, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_base_384_w12_22kto1k(input_tensor = None, input_shape = None, window_size = 12, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_large_224_w7_22k(input_tensor = None, input_shape = None, window_size = 7, classes = 21841, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = swin_transformer(img_input, classes, include_top, patch_size = 4, n_feature = 192, n_blocks = [2, 2, 18, 2], n_heads = [6, 12, 24, 48], window_size = window_size, ratio = 4., scale = None, use_bias = True, patch_normalize = True, dropout_rate = dropout_rate, attention_dropout_rate = attention_dropout_rate, droppath_rate = droppath_rate, normalize = normalize, activation = activation)
    model = tf.keras.Model(img_input, out)
...
def swin_transformer_large_224_w7_22kto1k(input_tensor = None, input_shape = None, window_size = 7, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_large_384_w12_22k(input_tensor = None, input_shape = None, window_size = 12, classes = 21841, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_large_384_w12_22kto1k(input_tensor = None, input_shape = None, window_size = 12, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_v2_tiny_256_w8_1k(input_tensor = None, input_shape = None, window_size = 8, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [8, 8, 8, 8], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_v2_tiny_256_w16_1k(input_tensor = None, input_shape = None, window_size = 16, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [16, 16, 16, 8], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_v2_small_256_w8_1k(input_tensor = None, input_shape = None, window_size = 8, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.3, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [8, 8, 8, 8], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = swin_transformer_v2(img_input, classes, include_top, patch_size = 4, n_feature = 96, n_blocks = [2, 2, 18, 2], n_heads = [3, 6, 12, 24], window_size = window_size, ratio = 4., use_bias = True, patch_normalize = True, dropout_rate = dropout_rate, attention_dropout_rate = attention_dropout_rate, droppath_rate = droppath_rate, normalize = normalize, activation = activation, pre_window_size = pre_window_size)
    model = tf.keras.Model(img_input, out)
...
def swin_transformer_v2_small_256_w16_1k(input_tensor = None, input_shape = None, window_size = 16, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.3, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [16, 16, 16, 8], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_v2_base_256_w8_1k(input_tensor = None, input_shape = None, window_size = 8, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.5, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [8, 8, 8, 8], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = swin_transformer_v2(img_input, classes, include_top, patch_size = 4, n_feature = 128, n_blocks = [2, 2, 18, 2], n_heads = [4, 8, 16, 32], window_size = window_size, ratio = 4., use_bias = True, patch_normalize = True, dropout_rate = dropout_rate, attention_dropout_rate = attention_dropout_rate, droppath_rate = droppath_rate, normalize = normalize, activation = activation, pre_window_size = pre_window_size)
    model = tf.keras.Model(img_input, out)
...
def swin_transformer_v2_base_256_w16_1k(input_tensor = None, input_shape = None, window_size = 16, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.5, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [16, 16, 16, 8], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_v2_base_192_w12_22k(input_tensor = None, input_shape = None, window_size = 12, classes = 21841, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [12, 12, 12, 6], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_v2_base_256_w16_22kto1k(input_tensor = None, input_shape = None, window_size = 16, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [12, 12, 12, 6], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_v2_base_384_w24_22kto1k(input_tensor = None, input_shape = None, window_size = 24, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [12, 12, 12, 6], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_v2_large_192_w12_22k(input_tensor = None, input_shape = None, window_size = 12, classes = 21841, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [12, 12, 12, 6], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = swin_transformer_v2(img_input, classes, include_top, patch_size = 4, n_feature = 192, n_blocks = [2, 2, 18, 2], n_heads = [6, 12, 24, 48], window_size = window_size, ratio = 4., use_bias = True, patch_normalize = True, dropout_rate = dropout_rate, attention_dropout_rate = attention_dropout_rate, droppath_rate = droppath_rate, normalize = normalize, activation = activation, pre_window_size = pre_window_size)
    model = tf.keras.Model(img_input, out)
...
def swin_transformer_v2_large_256_w16_22kto1k(input_tensor = None, input_shape = None, window_size = 16, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [12, 12, 12, 6], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def swin_transformer_v2_large_384_w24_22kto1k(input_tensor = None, input_shape = None, window_size = 24, classes = 1000, include_top = True, dropout_rate = 0., attention_dropout_rate = 0., droppath_rate = 0.2, normalize = normalize, activation = tf.keras.activations.gelu, pre_window_size = [12, 12, 12, 6], weights = "imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape = input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
